...
#region Unity Messages
	...
	private void UpdateAnchors()
	{
		...
		if (!float.IsNaN(hmdLeftEye.position.x) && !float.IsNaN(hmdLeftEye.position.y) && !float.IsNaN(hmdLeftEye.position.z) &&
		    !float.IsNaN(hmdRightEye.position.x) && !float.IsNaN(hmdRightEye.position.y) && !float.IsNaN(hmdRightEye.position.z)) {
